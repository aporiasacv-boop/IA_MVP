...
from app.reasoning_engine.metrics import reset_metrics
...
def test_hybrid_chat_adds_reasoning_metadata_without_changing_answer() -> None:
    reset_metrics()
    ...
    assert result.metadata["reasoning_route"] == "conversation"
    assert "reasoning_actual_route" in result.metadata
    assert "reasoning_route_match" in result.metadata
